...
const Basket = () => {
    ...
    const goods = Number(
        basket.reduce((acc, elem) => acc + (elem.price ?? 0), 0).toFixed(2)
    );
    const delievery = Math.floor(Math.random() * 3) + 4;
    const total = (goods + delievery).toFixed(2);
    ...
    return (
        <div className={'py-9 px-3 md:p-9 flex flex-col items-center h-full'}>
            ...
            <div className={'min-h-[50vh] w-full md:w-1/2 h-full flex flex-col justify-center items-center'}>
                {
                    basket.length === 0 ?
                        <p className={'text-lg font-medium'}>Basket is empty</p> : basket.map((elem) => (
                            <BasketItem elem={elem} key={elem.id} onRemove={() => removeFromBasket(elem.id)}/>
                        ))
                }
                {
                    basket.length !== 0 && (
                        <div className={'w-full mt-20 md:mt-10'}>
                            ...
                            <div className={'flex justify-between'}>
                                ...
                                <p className={'font-medium mb-4'}>${delievery}</p>
                            </div>
                            <Separator/>
                            <div className={'flex justify-between'}>
                            ...
                            <Button className={'mt-10 w-full rounded-full p-5 font-semibold'} variant={'secondary'}>
                                Proceed to payment
                            </Button>
                        </div>
                    )
